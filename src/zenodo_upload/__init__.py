...
@click.option("--record-id", "--deposit-id", "--to", help="Zenodo record ID to upload files to", type=str, default=None, required=True)
...
class ZenodoUploader:
    ...
    def upload_files_to_record(self, record_id: str, file_paths: List[str], max_retries: int = 3) -> None:
    ...
    def _upload_single_file(self, bucket_url: str, file_path: str, max_retries: int) -> None:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    click.echo(f"Upload failed for {file_name}, retrying in {wait_time} seconds... (attempt {attempt + 1}/{max_retries + 1})")
                    time.sleep(wait_time)
                else:
                    click.echo(f"Failed to upload {file_name} after {max_retries + 1} attempts", err=True)
                    raise
